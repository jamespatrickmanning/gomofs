...
@author: Xiaojian
modified by JiM in Aug 2017
-put hardcodes at top
"""
import numpy as np
...
yr=2005;mth=5;day=5;# start time
days=3 # number of days to run
URL1='current_'+str(yr-2000).zfill(2)+'hind_hourly.nc'
...
jia=14*0  # what is "jia"

start_time=datetime(yr,mth,day,0,0,0,0)
...
#########################################################
# we set velocity to zero when on land 
uu=[] # what is this for?  
vv=[]
b=0
...
np.save('roms'+str(yr)+'_',str(mth)+'_',str(days),roms)
plt.legend(loc='best') 
plt.plot(CL['lon'],CL['lat'],'b-',linewidth=0.5) 
plt.xlim([-69.5,-64.75])
plt.ylim([43.5,45.33])
plt.savefig('roms_'+str(yr)+'_',str(mth)+'_',str(day),dpi=400)
